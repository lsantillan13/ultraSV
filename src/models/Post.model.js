...
const CATEGORY_MAP = {
  "anticipacion-politica": "Anticipación Política", "politica": "Política", "internacionales": "Internacionales",
  "policiales": "Policiales", "judiciales": "Judiciales", "seguridad": "Seguridad",
  "sociedad": "Sociedad", "ciudad": "Ciudad", "region": "Región", "infraestructura": "Infraestructura", "obras": "Obras", "rutas": "Rutas", "transito-y-transporte": "Tránsito y Transporte", "clima": "Clima", "cooperativas": "Cooperativas", "vivienda-y-habitat": "Vivienda y Hábitat", "inmobiliarias": "Inmobiliarias",
  "economia": "Economía", "emprender": "Emprender", "vaca-muerta": "Vaca Muerta", "energia": "Energía", "campo-y-produccion": "Campo y Producción", "trabajo": "Trabajo", "gremiales": "Gremiales", "jubilados-y-anses": "Jubilados y ANSES",
  "salud": "Salud", "educacion": "Educación", "ciencia-y-tecnologia": "Ciencia y Tecnología", "tecnologia": "Tecnología", "deportes": "Deportes", "deporte-local": "Deporte Local", "gaming-y-esports": "Gaming y Esports",
  "espectaculos": "Espectáculos", "los40": "Los40", "streaming": "Streaming", "cultura": "Cultura", "redes": "Redes", "lifestyle": "Lifestyle", "gastronomia": "Gastronomía", "turismo": "Turismo", "ambiente": "Ambiente", "mascotas": "Mascotas", "genero-y-diversidad": "Género y Diversidad", "servicio-feriados": "Servicio / Feriados", "institucional": "Institucional", "agenda": "Agenda", "loteria-y-quiniela": "Lotería y Quiniela"
};
...
const stripHtml = (html = '') => html.replace(/<style[^>]*>.*?<\/style>/gis, '').replace(/<script[^>]*>.*?<\/script>/gis, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
...
const postSchema = new Schema({
  ...
  carouselMain: { type: Boolean, default: false, index: true }, // 1 grande
  carouselMainAt: { type: Date, default: null },
  carouselSide: { type: Boolean, default: false, index: true }, // 4 chicas
  carouselOrder: { type: Number, default: 0, min: 0, max: 3 },
  carouselSideAt: { type: Date, default: null },

  // --- 5 DESTACADAS OPCIONAL ---
  destacada: { type: Boolean, default: false, index: true },
  destacadaOrder: { type: Number, default: 0, min: 0, max: 4 },
  destacadaAt: { type: Date, default: null },

  Entry_Slug: { type: String, trim: true, lowercase: true, unique: true, sparse: true },
  ...
  Entry_Tags: {
    ...
    set: (tags) => {
      ...
      return [...new Set(tags.map(t => t.toString().toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9-]/g, '-').replace(/--+/g, '-').replace(/^-|-$/g, '')).filter(Boolean))].slice(0, 12);
    }
  },
...
postSchema.pre('save', function(next){
  ...
  if (this.Entry_Tags.length === 0 && this.Entry_Title) {
    ...
    if (words.length) { this.Entry_Tags = words; this.Entry_Tags_Auto = true; }
  }
  next();
});
...
postSchema.index({ carouselMain: 1, carouselMainAt: -1 });
postSchema.index({ carouselSide: 1, carouselOrder: 1 });
postSchema.index({ destacada: 1, destacadaOrder: 1 });
